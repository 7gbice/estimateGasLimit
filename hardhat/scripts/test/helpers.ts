...
export async function createOrUpdateConfigFile(
  ...
  const deployedContracts = "./scripts/generatedTransferUSDCData.json"; // Path to the config file.
  ...
  if (fs.existsSync(deployedContracts)) {
    ...
    if (sourceAddress) config[network].transferUSDC = sourceAddress;
    if (destinationAddress) config[network].transferUSDC = destinationAddress;
  } else {
